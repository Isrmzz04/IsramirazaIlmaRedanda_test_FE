...
import { getEmployeeById } from "@/store/employees/action"
...
const DetailEmployee = ({ params }) => {
  ...
  useEffect(() => {
    if (dataEmployee) {
      setFormData({
        ...
        jobs: dataEmployee.jobs || "",
        shift: dataEmployee.shift || "",
        status: dataEmployee.status || false,
      });
    }
  }, [dataEmployee]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await httpService.put(`/employees/${params.employeeId}`, formData)
  ...
  return (
    <div>
      <Heading>Detail Karyawan {params.employeeId}</Heading>
      <div>
